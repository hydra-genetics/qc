...
from somalier_mqc_config import comment_the_config_keys, process_sample_file
...
class TestSomalierMqcConfigMain(unittest.TestCase):
    ...
    def test_full_workflow(self):
        ...
        snakemake.input.samples = sample_file
        snakemake.output = [output_file]
        snakemake.log = [os.path.join(self.test_dir, "test.log")]

        # Run the main function
        from somalier_mqc_config import main
        main()

        # Compare with expected output (skip commented config lines as they may have formatting differences)

...
rule picard_hs_metrics:
    ...
    output:
        temp("qc/picard_hs_metrics/{sample}_{type}.HsMetrics.txt"),
    params:
        extra=config.get("picard_hs_metrics", {}).get("extra", "COVERAGE_CAP=5000"),
    log:
        "qc/picard_hs_metrics/{sample}_{type}.log",
    benchmark:
        repeat(
            "qc/picard_hs_metrics/{sample}_{type}.benchmark.tsv", config.get("picard_hs_metrics", {}).get("benchmark_repeats", 1)
        )
